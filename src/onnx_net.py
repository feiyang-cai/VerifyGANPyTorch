...
#def rebuild_matmul(node, weights):
#    def matmul(a, b):
#        print(b.shape, a.shape, weights)
#        return torch.matmul(b, a.transpose(0, 1))
#    return matmul, node.input, node.output
...
class ONNXNet(nn.Module):
    ...
    def forward(self, x):
        return self.controller(self.main(x))

def convert_onnx2pytorch(onnx_filepath="./models/papermodel/full_mlp_supervised.onnx"):
    net = ONNXNet()
    onnx_model = onnx.load(onnx_filepath)
    weights = unpack_weights(onnx_model.graph.initializer)
    for o_weight, p_weight in zip(weights, net.main.parameters()):
        p_weight.data = weights[o_weight]
    torch.save(net.main.state_dict(), "./models/papermodel/full_mlp_supervised.pth")
    net.controller.weight.data = torch.FloatTensor([[-0.74, -0.44]])
    torch.save(net.state_dict(), "./models/papermodel/AllInOne.pth")

    x = torch.randn(1, 4)

    torch.onnx.export(net, 
                      x,
                      "./models/papermodel/AllInOne.onnx",
                      input_names = ['input'],   # the model's input names
                      output_names = ['output']) # the model's output names


if __name__ == "__main__":
    convert_onnx2pytorch()
